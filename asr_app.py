...
st.set_page_config(page_title="Chichewa ASR & Translation", layout="wide")
...
def load_models():
    ...
    translation_model = pipeline("translation", model="zerolat3ncy/nllb-600M-chichewa-english-financial", device="cpu")
...
if input_method == "Upload Audio File":
    uploaded_file = st.file_uploader("Upload Chichewa audio", type=['wav', 'mp3', 'ogg', 'flac', 'm4a'])
    if uploaded_file is not None:
        st.audio(uploaded_file)
        audio_data = uploaded_file
else:
    recorded_audio = st.audio_input("Record Chichewa audio")
...
with col1:
    reference_Chichewa = st.text_area("Reference Chichewa (optional)", placeholder="Enter correct Chichewa text")
